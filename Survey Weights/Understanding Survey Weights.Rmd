...
data <- purrr::map2_df(
  .x = proportions_dfs,
  .y = list(c(.55, .45), c(.25, .05, .8), c(.75, .25), c(.2, .1, .70)),
...
Now let's double check that our data is of the class `data.frame` otherwise the `anesrake` function will error out, and then apply the weights!
...
rake_output <- anesrake::anesrake(
  ...
  type = 'pctlim',       # What types of targets should be used to weight
  pctlim = 0.05          # Deviation threshold
)
```

Our `rake_output` object is a list containing the names of variables we used, the design effect, and data.frames comparing the unweighted proportions to the new weighted proportions. For this tutorial, though, we'll check our weights using our previous method for consistency and diversity in methods. 
